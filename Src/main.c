...
//
			...
			UARTsendString(Tool_MY_USART,(uint8_t*)gsPrintfBuffer);
...
void periph_init(void)/*{{{*/
{
	...
	UARTsendString(Tool_MY_USART,"hello\r\n");
#ifdef Test_MY_USART/*{{{*/
	while(1)
	{
		{
...
void PrintProcess(void)/*{{{*/
{
	if(gSend == 1)/*{{{*/
	{
		...
		if(gSend == 2)
		{
			UARTsendString(Tool_MY_USART,(uint8_t*)gUARTpointer);
			gSend = 0;
		}else if(gSend == 3)
		{
			UARTsendN(Tool_MY_USART,(uint8_t*)gUARTpointer,gsPrintfActualLength);
			gSend = 0;
		}
	}/*}}}*/
}/*}}}*/
